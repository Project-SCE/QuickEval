...
const EditEvaluatorForm = ({handleEditClose, onSubmit, onClose, currentEvaluator }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
        ...
        const id = currentEvaluator._id;
       // Make PUT request to update the item

       const response = await axios.put(`${serverUrl}/evaluators/${id}`, dataToSend);
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center">
      <div ref={formRef} className="bg-gray-400 p-5 rounded-lg shadow-lg w-1/3">
        <div className="text-xl mb-4 flex justify-center font-bold text-black">Edit Evaluator</div>
